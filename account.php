...
// Ensure history row exists (for in-progress exam)
$check2 = mysqli_query($con,"SELECT * FROM history WHERE email='$email' AND eid='$eid' LIMIT 1");

if(mysqli_num_rows($check2) == 0){
    $start_time = time();
    $end_time = $start_time + ($exam_duration_minutes * 60); // replace with actual duration
    mysqli_query($con,"INSERT INTO history (email,eid,start_time,end_time) VALUES('$email','$eid',$start_time,$end_time)") or die(mysqli_error($con));
    error_log("[DEBUG] Created in-progress history row for email=$safeEmail, eid=$safeEid, start_time=$start_time, end_time=$end_time");
} else {
    $row = mysqli_fetch_assoc($check2);
    $end_time = $row['end_time'];
    error_log("[DEBUG] Using existing history row end_time=$end_time");
}
...
<?php } else { ?>
 <tr style="color:#99cc32">
  ...
  <td>
    <?= $title ?>
    <span title="You have already taken this exam" class="glyphicon glyphicon-ok" aria-hidden="true"></span>
  </td>
  <td><?= $total ?></td>
  <td><?= $sahi * $total ?></td>
  <td><?= $time ?>&nbsp;min</td>
...
if (mysqli_num_rows($check) > 0) {
    ...
    if ((intval($row['score']) > 0) || ($row['end_time'] <= time())) {
        $alreadyTaken = true;
    }
}
